...
class TestSample(unittest.TestCase):
    ...
    def test_scatter(self):
        ...
        print ('@@@@@@     line: 22  - ', rms, pix_var, sample.actual_points, sample.sector_area)

        sample = Sample(test_data, sma, astep=astep)
        sample.update()
        rms = np.std(sample.values[2])
        # pix_var = rms * np.sqrt(np.sqrt(sample.sector_area))
        pix_var = rms * np.sqrt(sample.sector_area)
        print ('@@@@@@     line: 28  - ', rms, pix_var, sample.actual_points, sample.sector_area)
        ...
        # pix_var = rms * np.sqrt(np.sqrt(sample.sector_area))
        pix_var = rms * (np.sqrt(sample.sector_area))
        print ('@@@@@@     line: 34  - ', rms, pix_var, sample.actual_points, sample.sector_area)
